...
- Give every variable a sensible, realistic default value matching its declared type — EXCEPT for a variable that uniquely identifies one specific device or instance and has no genuine typical value across deployments: serial numbers, asset tags, MAC addresses, license/activation keys, and specific per-site IPs, hostnames, or device names. For those, use an empty string ("") as the default instead of inventing a plausible-looking one. A made-up serial number or IP is indistinguishable from a real one and will be mistaken for one; the person generating this template has no actual value to give it, and a fabricated example that looks legitimate is worse than an honestly empty field. Reserve realistic defaults for values with a genuine common convention across most setups — a standard port, a typical VLAN ID, a default config path, a common timeout — where "typical" means something real, not "plausible enough to pass as an example."
- For a value that would normally be read off a physical label or looked up in an external system rather than typed from memory (a device's serial number/asset tag, a specific IP from IPAM/DHCP records, an entry in an asset inventory), phrase the step to say where it comes from — "record the serial number printed on the device's rear label" / "look up the IP assigned to {{device_name}} in your IPAM system" — not just a bare {{field}} with no indication of its source.
...
- variables[].default should be the original/current value found in the document at that spot where one exists — but if you're parameterizing a value that uniquely identifies one device/instance (a serial number, asset tag, MAC address, license key, or a specific per-site IP/hostname/device name) and the document didn't already contain a real one, use an empty string ("") rather than inventing a plausible-looking example. A fabricated identifier that looks real is worse than an honestly empty field. For a value that would normally come from a physical label or an external system of record, phrase the step to say where it comes from rather than leaving a bare unexplained field.
