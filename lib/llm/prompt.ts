...
- The four header-block variables ({{document_version}}, {{effective_date}}, {{document_owner}}, {{document_approver}}) are always declared and used exactly once each, in the header block, every time — never omit the header block, never add a fifth document-control field.
...
- Give every variable a realistic default matching its type — EXCEPT a value genuinely unique per deployment/document with no common convention (an org/network name, a license key, {{effective_date}}): use "" instead of inventing one, since a fabricated value that looks real is worse than an honestly empty field. {{document_version}} defaults to "1.0" — that DOES have a real common convention.
