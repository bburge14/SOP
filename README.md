...
- **Flagged assumptions** are a fourth generation-quality rule, added directly from a live report: a generated SOP for resetting an internal, custom RiseVision-based TV setup presented an invented Linux/systemd/SSH-based reset mechanism (a specific service name, a specific config file path) as settled fact, with no way to tell that part was a guess rather than something confirmed — and it genuinely might not have matched the real hardware at all. This is a different problem from a site-specific value: a `{{variable}}` is for something that legitimately varies while the *approach* stays correct (an IP, a hostname); this is for a case where the AI can't verify whether the approach itself is even right for this setup. `SOP_SYSTEM_PROMPT` and `REVIEW_IMPROVE_SYSTEM_PROMPT` now instruct the model to mark this kind of inferred-not-confirmed fact with a GitHub-style alert (`> [!WARNING]`) instead of stating it as fact, reinforced again in `template_markdown`'s JSON-Schema description. Rendering support (`lib/sop/remarkGithubAlerts.ts`) turns `> [!WARNING]`/`[!CAUTION]`/`[!IMPORTANT]`/`[!NOTE]`/`[!TIP]` blockquotes into a distinctly colored callout — a real `<div class="sop-alert-*">`, not just a plain blockquote with different text, verified with a remark→rehype pipeline to confirm the marker is stripped from visible content and normal blockquotes without the marker are left untouched. `markdownToDocx.ts` gives exported documents the same distinct treatment (a colored bold label + colored border) so the flag survives being handed to someone as a .docx — verified by unzipping a generated file and confirming the label/color/body text are all present and the raw `[!WARNING]` marker isn't. Doesn't round-trip back through `docxToMarkdown.ts` on re-import (degrades to a plain blockquote, losing the marker) — an accepted, bounded gap rather than something this pass also solves. Verified against a real local model (Ollama) using the actual RiseVision example that prompted this feature: fed it back through Review & Improve, and — without being told what to flag — it added `> [!WARNING] This assumes the RiseVision Player service name (risevision-player) and configuration file path (/etc/risevision/player.conf) are correct. Verify these match your setup before relying on them`, which then rendered correctly as a real warning callout through the app's actual pipeline, not just a hand-written test case.
