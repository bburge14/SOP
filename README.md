...
- **Only the topic string is ever sent to the AI provider by default — with two explicit, opt-in exceptions.** `app/api/generate/route.ts` accepts `{ topic, context? }` — the values you type into the variable form (hostnames, IPs, credentials, anything site-specific) are substituted entirely client-side by `lib/sop/template.ts#renderTemplate` and never leave the browser/app unless you explicitly export/copy. `TopicInput.tsx` shows a standing reminder not to put proprietary, confidential, or personal information in the topic itself, since that string does go to a third party. The first exception is the "Scan with AI" button (`app/api/analyze-import/route.ts`), which exists to let the AI read an *imported* document and parameterize it — that necessarily means sending the document's full rendered content, not just a topic. It's never called automatically; `SopWorkspace.tsx#handleAnalyzeWithAi` shows a `window.confirm` spelling out exactly that trade-off (and warning against proprietary content) before every single call, with no "don't ask again." The second is **Attach Reference** (see Notable behavior below) — files you deliberately attach for generation are sent in full alongside the topic; the privacy notice under the topic input names this explicitly whenever any files are attached, rather than only warning about the topic string.
